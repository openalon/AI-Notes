...
这和 [[Anthropic：The AI-native SDLC playbook]] 的硬边界同构：写的身份不能批自己；检测保持确定性；人留在闸门上。Warp 把同一条用在 **skill 文件的进化** 上：inner 可以无人启动，outer 可以自己开 PR，**合入必须是人。** 指令如果驱动生产，「它们就该住在仓库里，有版本历史、审查和回滚。」
