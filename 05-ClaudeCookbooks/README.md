...
<summary>出处</summary>
<p>来源：<a href="https://github.com/anthropics/claude-cookbooks">anthropics/claude-cookbooks</a> · 开始时间：2026-09-12</p>
</details>
...
| [x] | 第 3 章 | 工具调用：何时调用、并行、代码里调工具、工具搜索 |
...
1. **对着 notebook 跑，不对着笔记抄代码**：判断已经写进各章；实现留在官方仓库。
2. **和 Harness 对照**：Book 1 讲 Claude Code 的控制面；这里讲 Messages API / Agent SDK / 托管 Agents 把同一套约束拆成可复制的 recipe。
3. **官方食谱表不是全书**：仓库里还有未进 `registry.yaml` 的 notebook（例如部分托管 Agents 教程）。章节清单会标出来。
