...
未读导读也要有 `## 一句话`，哪怕先写「（阅读后填写）」。不要只留三个填写块。
...
未读章先占位：cite 写「状态：未读」，填写块可以暂时写「（阅读后填写）」，但必须有 `id`、标题、cite、**一句话**、核心概念、关键洞察、个人思考、**相关**（导读 + 上一章 + 下一章）。官方目录里有的附录也要占位，不要只写在 README 进度表里。不要用文章模板的 `title` / `tags` / `source` / `created` YAML——书的出处在 cite 芯片里，网页评论只认 `id`。助手之间可以说骨架；贴进网页的文件写「占位」。

`id` 必须是 `n_` + 12 位 hex（`n_` + `secrets.token_hex(6)`）。不要写成 `n_book1_appa` 这种可读别名：Giscus 按这个字符串挂评论，格式不统一以后对不上。写入后永不改。已经写成别名、且确定还没有评论线程时，改成合法 hex。
...
- 文件名、H1、wikilink 三者能对上；basename 全库唯一（阅读导读带书名短前缀，不要再用第二个 `00-阅读导读.md`）
- `id` 已写入、是 `n_` + 12 位 hex、且未复用
...
- 不要在对话里总结完一章却不改文件。
- 不要把未读章建成「只有三个填写块、没有一句话和相关」的半文件；官方附录不要只出现在 README。
