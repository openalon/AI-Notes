...
| 进度 | 章节 | 主题 |
|------|------|------|
| [x] | 阅读导读 | 如何理解第一本书与这本比较书（文件：`00-Book2阅读导读`） |
| [x] | 序言 | 两套 Harness，不必假装是同一匹马的附件 |
| [x] | 第 1 章 | 为什么要把 Claude Code 和 Codex 放在一起看 |
| [x] | 第 2 章 | 两种控制面：Prompt 拼装与 Instruction Fragment |
| [x] | 第 3 章 | 心跳放在哪：Query Loop 对照 Thread、Rollout 与 State |
| [x] | 第 4 章 | 工具、沙箱与策略语言：谁来阻止模型动手太快 |
| [x] | 第 5 章 | 技能、Hook 与本地规则：系统如何学会守乡约 |
| [x] | 第 6 章 | 委派、验证与持久状态：谁来防止系统自己给自己打高分 |
| [x] | 第 7 章 | 殊途同归，还是各表一枝 |
| [x] | 第 8 章 | 如果你要自己做：该向谁学，先学什么 |
| [x] | 附录 A | 源码索引：这套比较主要依据哪些文件 |
| [x] | 附录 B | 检查清单：如何判断你的 Harness 更像 Claude Code、Codex，还是半成品 |
